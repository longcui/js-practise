...
test('anonymous function', () => {
    ...
    // expect(fun3(5, 5)).toBe({ a: 5, b: 5 });    //error
    //Compared values have no visual difference. 
    //Note that you are testing for equality with the stricter`toBe` matcher using`Object.is`.For deep equality only, use`toEqual` instead. < Click to see difference >
    expect(fun3(5, 5)).toEqual(expected);

    // Parenthesize the body of function to return an object literal expression:
    const fun4 = (x, y) => ({ a: x, b: y });
    // expect(fun3(5, 5)).toBe({a: 5, b: 5});  
